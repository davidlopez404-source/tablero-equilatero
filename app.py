...
fig.add_trace(go.Scatter(
    x=df['Mes'], y=[float(val) for val in df['Costos Totales']],
    mode='lines',
...
fig.add_trace(go.Scatter(
    x=df['Mes'], y=[float(val) for val in df['MRR']],
    mode='lines',
...
fig.update_layout(
    ...
    yaxis=dict(title='USD ($)', showgrid=True, gridcolor='rgba(0, 255, 204, 0.13)'),
    hovermode="x unified",
